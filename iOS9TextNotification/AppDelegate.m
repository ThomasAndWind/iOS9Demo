...
- (void)application:(UIApplication *)application handleActionWithIdentifier:(NSString *)identifier forLocalNotification:(UILocalNotification *)notification withResponseInfo:(NSDictionary *)responseInfo completionHandler:(void (^)())completionHandler {
    ...
    if ([identifier isEqualToString:@"test_text_action"]) {
        
        NSDictionary *dic = @{@"text":[responseInfo objectForKey:UIUserNotificationActionResponseTypedTextKey]};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"text" object:nil userInfo:dic];
        completionHandler(UIBackgroundFetchResultNoData);
    }else {
        NSDictionary *dic = @{@"text":@"在路上!!!"};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"text" object:nil userInfo:dic];
        completionHandler(UIBackgroundFetchResultNoData);
    }
}
